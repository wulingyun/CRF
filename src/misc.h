...
#include <R_ext/RS.h>   // for R_Calloc, Calloc, R_Free
#include <R.h>
#include <Rdefines.h>
#include <Rinternals.h>
#include <R_ext/Utils.h>  // for R_Calloc and R_Free
#include <cstring>
...
inline T **C_allocArray(int dim[n])
{
    ...
    T **array, **sub1, **sub2, **tmp;
    size1 = dim[0];
    array = sub1 = (T **) R_Calloc(size1, T*);
    for (int i = 1; i < n-1; i++)
    {
        size2 = size1 * dim[i];
        tmp = sub2 = (T **) R_Calloc(size2, T*);
        for (int j = 0; j < size1; j++)
        {
            sub1[j] = (T *) tmp;
            tmp += dim[i];
    ...
    T *block = (T *) R_Calloc(size1 * dim[n-1], T);
    for (int i = 0; i < size1; i++)
    {
        sub1[i] = block;
        block += dim[n-1];
...
inline T **R_allocArray2(int dim1, int *dim2)
{
    ...
    for (int i = 0; i < dim1; i++)
        if (dim2[i] >= 0) array_size += dim2[i]; else { array_size = -1; break; }
...
inline T **C_allocArray2(int dim1, int *dim2)
{
    ...
    block = (T *) R_Calloc(array_size, T);
    array = (T **) R_Calloc(dim1, T*);
    for (int i = 0; i < dim1; i++)
    {
        array[i] = block;
        block += dim2[i];
