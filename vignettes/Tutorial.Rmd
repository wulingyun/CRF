...
In order to learn Markov random field model from generated data, we first build another CRF object:
```{r}
...
Now we try to learn a CRF model from HMM data. We first build another CRF object:
```{r}
...
Then we train the model using `train.crf` function:
```{r}
...
With trained CRF model, we can infer the hidden states given an observation:
```{r}
...
```
Compared with the true hidden states:
```{r}
